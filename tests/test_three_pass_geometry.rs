...
mod three_pass_tests {
    ...
    fn test_three_pass_relation_geometry_with_rome() {
        ...
        for line in lines {
            ...
            if json["type"] == "relation" {
                ...
                if json.get("centroid").is_some() && json.get("bounds").is_some() {
                    ...
                    assert!(
                        centroid["lat"].is_string() || centroid["lat"].is_number(),
                        "Centroid should have lat field"
                    );
                    assert!(
                        centroid["lon"].is_string() || centroid["lon"].is_number(),
                    ...
                    assert!(
                        bounds["n"].is_string() || bounds["n"].is_number(),
                        "Bounds should have north field"
                    );
                    assert!(
                        bounds["s"].is_string() || bounds["s"].is_number(),
                        "Bounds should have south field"
                    );
                    assert!(
                        bounds["e"].is_string() || bounds["e"].is_number(),
                        "Bounds should have east field"
                    );
                    assert!(
                        bounds["w"].is_string() || bounds["w"].is_number(),
                        "Bounds should have west field"
                    );

                    println!(
                        "✅ Found relation {} with geometry: centroid={}, bounds={}",
        ...
        println!(
            ...
            if found_relation_with_geometry {
                "Yes"
            } else {
                "No"
            }
    ...
    fn test_geometry_mode_selection() {
        ...
        assert!(
            file_size_gb < 1.0,
            "Rome file should be small enough for full geometry mode"
        );
        ...
        if !output_content.trim().is_empty() {
            ...
            let has_geometry = output_content
                .lines()

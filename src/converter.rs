...
pub fn convert_pbf_to_geojson_with_geometry_level(
    ...
    if use_full_geometry {
        ...
        if file_size_gb < 0.1 {
            eprintln!("Very small file, attempting three-pass processing with relation geometry...");
...
fn convert_pbf_with_full_geometry(
    ...
    eprintln!("Collected {} node coordinates ({:.1}MB memory)",
              all_nodes.len(),
                ...
                if feature_count % 50000 == 0
                    && let Some(memory_usage) = get_memory_usage_mb()
                        && memory_usage > MEMORY_LIMIT_GB * 1024 {
                            eprintln!("⚠️  Memory usage ({} MB) exceeds limit ({} GB)",
                                     memory_usage, MEMORY_LIMIT_GB);
                        }
            }

            writer.flush()?;
            eprintln!("Streaming output complete. Total features: {}", feature_count);
            Ok(())
        })
    };
                ...
                let json_opt = match &osm_element {
                    ...
                    OsmElement::Way(way) => {
                        if !way.tags.is_empty() {
                            convert_way_to_json_with_full_geometry(way, &all_nodes_clone, pretty_print)
                        } else {
                            None
                        }
                    }
                    OsmElement::Relation(relation) => {
                        if !relation.tags.is_empty() {
                            convert_relation_to_json_with_full_geometry(relation, &all_nodes_clone, pretty_print)
                        } else {
                            None
                        }
                    }
...
fn process_element(element: Element, tag_filter: &Option<Vec<String>>) -> Option<OsmElement> {
...
fn calculate_bounds(coordinates: &[(f64, f64)]) -> Bounds {
    if coordinates.is_empty() {
        return Bounds { north: 0.0, south: 0.0, east: 0.0, west: 0.0 };
    }
...
fn convert_pbf_with_complete_geometry(
    ...
    eprintln!("Collected {} way geometries ({:.1}MB memory)",
              all_ways.len(),
              all_ways.len() as f64 * 200.0 / 1_048_576.0); // Estimate ~200 bytes per way
        ...
        |element| {
            ...
            if let Some(osm_element) = process_element(element, &tag_filter_clone) {
                let json_opt = match &osm_element {
                    ...
                    OsmElement::Relation(relation) => {
                        if !relation.tags.is_empty() {
                            convert_relation_to_json_with_way_resolution(relation, &all_ways_clone, pretty_print)
                        } else {
                            None
                        }
                    }
    ...
    let ways = reader.par_map_reduce(
        |element| {
            ...
            match element {
                Element::Way(way) => {
                    let node_refs: Vec<i64> = way.refs().collect();
                    let coordinates: Vec<(f64, f64)> = node_refs.iter()
                ...
                _ => {} // Skip nodes and relations in pass 2
            }
            local_ways
        },
        HashMap::new,
...
fn convert_relation_to_json_with_way_resolution(
    ...
    if !all_coordinates.is_empty() {
        ...
        let members_json: Vec<serde_json::Value> = relation.members.iter().map(|member| {
            json!({
                "type": match member.member_type {
                    MemberType::Node => "node",
                    MemberType::Way => "way",
...
fn convert_way_to_json_with_full_geometry(
    ...
    let coordinates: Vec<(f64, f64)> = way.node_refs.iter()
        .filter_map(|node_id| all_nodes.get(node_id).cloned())
        .collect();
